<?php
/**
 * Content Warning taxonomy archive — Illusia Override
 *
 * Redesenho do archive de avisos de conteúdo com header editorial
 * semântico, tax cloud e card list Illusia.
 *
 * @package Illusia Theme
 * @since 1.11.0
 * @see fictioneer/taxonomy-fcn_content_warning.php (template pai)
 */
...
$taxonomy_slug = 'fcn_content_warning';
$taxonomy_label = __( 'Content Warning', 'fictioneer' );
$taxonomy_color = 'crimson';
...
<main id="main" class="main archive illusia-list-page illusia-archive illusia-archive--<?php echo esc_attr( $taxonomy_color ); ?> content-warning-archive">
    ...
    <article class="archive__article illusia-archive__article">
      ...
      <?php if ( ! empty( $tax_cloud ) ) : ?>
        <nav class="illusia-archive__cloud illusia-archive__cloud--<?php echo esc_attr( $taxonomy_color ); ?>"
             aria-label="<?php esc_attr_e( 'Related content warnings', 'fictioneer' ); ?>">
...
<?php
  $footer_args = array(
    ...
    'breadcrumbs' => array(
      ...
      [ sprintf( __( 'Warning: %s', 'fictioneer' ), single_tag_title( '', false ) ), null ],
    ),
  );
