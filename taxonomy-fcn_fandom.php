<?php
/**
 * Fandom taxonomy archive — Illusia Override
 *
 * Redesenho do archive de fandoms com header editorial semântico,
 * tax cloud como fichas de referência cruzada e card list Illusia.
 *
 * @package Illusia Theme
 * @since 1.11.0
 * @see fictioneer/taxonomy-fcn_fandom.php (template pai)
 */
...
$taxonomy_slug = 'fcn_fandom';
$taxonomy_label = __( 'Fandom', 'fictioneer' );
$taxonomy_color = 'violet';
...
<main id="main" class="main archive illusia-list-page illusia-archive illusia-archive--<?php echo esc_attr( $taxonomy_color ); ?> fandom-archive">
    ...
    <article class="archive__article illusia-archive__article">
      ...
      <?php if ( ! empty( $tax_cloud ) ) : ?>
        <nav class="illusia-archive__cloud illusia-archive__cloud--<?php echo esc_attr( $taxonomy_color ); ?>"
             aria-label="<?php esc_attr_e( 'Related fandoms', 'fictioneer' ); ?>">
...
<?php
  $footer_args = array(
    ...
    'breadcrumbs' => array(
      ...
      [ sprintf( __( 'Fandom: %s', 'fictioneer' ), single_tag_title( '', false ) ), null ],
    ),
  );
