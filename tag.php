<?php
/**
 * Tag archive — Illusia Override
 *
 * Redesenho do archive de tags com header editorial,
 * tax cloud e card list Illusia.
 *
 * @package Illusia Theme
 * @since 1.11.0
 * @see fictioneer/tag.php (template pai)
 */
...
$taxonomy_label = __( 'Tag', 'fictioneer' );
...
<main id="main" class="main archive illusia-list-page illusia-archive illusia-archive--<?php echo esc_attr( $taxonomy_color ); ?> tag-archive">
    ...
    <article class="archive__article illusia-archive__article">
      ...
      <?php if ( ! empty( $tax_cloud ) ) : ?>
        <nav class="illusia-archive__cloud"
             aria-label="<?php esc_attr_e( 'Related tags', 'fictioneer' ); ?>">
...
<?php
  $footer_args = array(
    ...
    'breadcrumbs' => array(
      ...
      [ sprintf( __( 'Tag: %s', 'fictioneer' ), single_tag_title( '', false ) ), null ],
    ),
  );
